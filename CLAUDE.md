...
- [x] `/backend-assistant` event store + assistant — **2026-09-09, rewritten by Member 2 given the Sep 10 deadline, with Member 3's authorization**: the code Member 3 sent (store.py/tools.py/assistant.py/offline_router.py/seed_data.py/test_assistant.py) was well-engineered — validated inserts, a clean function-calling tool layer, a deterministic offline router, 12 passing tests — but built against a schema he'd invented independently (`loading_bay`/`activity`/`behaviour="no_ppe"`/lowercase 3-tier `risk_level`), including a fabricated `root-CLAUDE.md` copy that didn't match the real one — no connection to what Member 2/4 actually built against all week. Rewrote schema.sql/store.py/tools.py/offline_router.py/seed_data.py/test_assistant.py onto the real schema (`event_id`/`behaviour_type`/`risk_level` Low-Critical/`risk_score`/`bay`/`object_ids`/`evidence_frame_path`/`explanation`, importing enums from `shared/config.py` so they can't drift again), keeping his architecture intact. All 13 tests pass on the real schema. **First real end-to-end run of the whole chain**: a real video → Member 1's CV pipeline → Member 2's behaviour engine → this store, confirmed by direct query, not just by absence of an exception. Wired `behaviour-risk/behaviour_risk_engine/event_sink.py` to call the real `insert_event()` (was a guessed, never-real import path before). (Member 2, on Member 3's behalf)
